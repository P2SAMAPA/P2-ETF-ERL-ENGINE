...
# ── Prompt Templates (as regular strings, to be formatted later) ───────────────

# The system prompt is defined as a regular string; it will be passed to Gemini
# directly when creating the model, so we need to format it now but we must ensure
# cfg is imported. Since we are after the import, it's safe.
...
You are managing a portfolio of {len(cfg.ASSETS)} ETFs ({', '.join(cfg.ALL_ASSETS)}), plus CASH.
...
# The reflection prompt uses placeholders that will be filled later
...
# The second attempt prompt also uses placeholders
...
# ── Helper to generate the weight adjustments string dynamically ───────────────
def _weight_adjustments_str() -> str:
    ...
    def generate_second_attempt_adjustment(
        ...
        if self.gemini.available and active_rules:
            prompt = SECOND_ATTEMPT_PROMPT_TEMPLATE.format(
                ...
                weight_adjustments_str = _weight_adjustments_str(),
            )
            raw = self.gemini.generate(prompt)
            if raw:
                parsed = self._parse_json_response(raw)
